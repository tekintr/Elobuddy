...
namespace AkaCore.Features.Activator.AItems
{
    class Qss : IModule
    {
        ...
        {
            if (AkaLib.Item.Qss.IsOwned() && AkaLib.Item.Qss.IsReady() && ObjectManager.Player.CountEnemyChampionsInRange(1800) > 0 && AkaCore.Manager.MenuManager.Qss)
            {
                Core.DelayAction(() => AkaLib.Item.Qss.Cast(), AkaCore.Manager.MenuManager.QssDelay);
            }

            if (AkaLib.Item.Mercurial.IsOwned() && AkaLib.Item.Mercurial.IsReady() && ObjectManager.Player.CountEnemyChampionsInRange(1800) > 0 && AkaCore.Manager.MenuManager.Mecurial)
            {
                Core.DelayAction(() => AkaLib.Item.Mercurial.Cast(), AkaCore.Manager.MenuManager.QssDelay);
            }

            if (AkaLib.Item.Cleanse != null && AkaLib.Item.Cleanse.IsReady() && ObjectManager.Player.CountEnemyChampionsInRange(1800) > 0 && AkaCore.Manager.MenuManager.Cleanse)
            {
