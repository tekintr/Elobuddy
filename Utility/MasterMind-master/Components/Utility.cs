...
namespace MasterMind.Components
{
    public sealed class Utility : IComponent
    {
        ...
        public void InitializeComponent()
        {
            ...
            Menu.AddLabel("Burada, yapilandirabileceginiz bazi kullanisli ve basit yardimci fonksiyonlari bulacaksiniz.");
            Menu.AddSeparator();

            Menu.AddGroupLabel("1. Clone Revealer");
            Menu.AddLabel("Sahte düsman sampiyonlarini Shaco klonu gibi capraz olarak ortaya koyar.");
            if (EntityManager.Heroes.Enemies.Any(o => CloneChampions.Contains(o.Hero)))
            {
                ShowClones = Menu.Add("crossClone", new CheckBox("Aktif"));
            ...
            {
                Menu.AddLabel(string.Format(" - Bu macta hicbir klon sampiyonu yok! ({0})", string.Join(" or ", CloneChampions)));
            }
            Menu.AddSeparator();

            Menu.AddGroupLabel("2. Juke Revealer");
            Menu.AddLabel("Reveals jukes, like flashing into brushes or Shaco Q");
            JukeRevealer = Menu.Add("juke", new CheckBox("Aktif"));
            JukeTimer = Menu.Add("jukeTimer", new Slider("Goster saka yonunu {0} saniyede", 3, 1, 10));
            Menu.AddLabel("Note: Once I'm able to check if the team has vision on the end position");
            Menu.AddLabel("Her zaman buyu cizmemektense gorus olmadiginda cizmekten vazgececegim.");
