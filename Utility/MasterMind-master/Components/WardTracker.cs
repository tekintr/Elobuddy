...
namespace MasterMind.Components
{
    public sealed class WardTracker : IComponent
    {
        ...
        public void InitializeComponent()
        {
            ...
            RenderWard = Menu.Add("renderWard", new CheckBox("Totemi haritada goster (Daire)"));
            DrawHealth = Menu.Add("drawHealth", new CheckBox("kalan totem canini goster"));
            DrawTime = Menu.Add("drawTime", new CheckBox("kalan totem suresini goster"));
            if (!MasterMind.IsSpectatorMode)
            {
                DrawAlliedWardTime = Menu.Add("drawAlliedTime", new CheckBox("dost totemlerin suresini goster"));
            }

            // Minimap options
            Menu.AddSeparator();
            Menu.AddLabel("Minimap options:");
            DrawMinimap = Menu.Add("drawMinimap", new CheckBox("Totem simgesi goster"));
            if (!MasterMind.IsSpectatorMode)
            {
            ...
            if (!MasterMind.IsSpectatorMode)
            {
                ...
                NotifyPlace = Menu.Add("notifyPlaceNear", new CheckBox("yakinlarda dusman totemi bulundugunu goster"));
                NotifyPlacePing = Menu.Add("notifyPlaceNearPing", new CheckBox("ping ile bildir", false));
                NotifyRange = Menu.Add("notifyPlaceNearRange", new Slider("Bildirim uyari araligi", 2000, 500, 5000));
            }
