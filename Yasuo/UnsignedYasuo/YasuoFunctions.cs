...
namespace UnsignedYasuo
{
    class YasuoFunctions
    {
        ...
        public static void Harrass()
        {
            ...
            if ((menu.GetCheckboxValue("Kullan Q") && !Yasuo.HasBuff("YasuoQ3W") && menu.GetCheckboxValue("Kullan Q minyonlara son vurus"))
                || (menu.GetCheckboxValue("Kullan Q3") && Yasuo.HasBuff("YasuoQ3W") && menu.GetCheckboxValue("Kullan Q3 minyonlara son vurus")))
        ...
        public static void Combo()
        {
            ...
            if (menu.GetComboBoxText("Hamle Modu: ") == "AtilmeOnleyici")
                EGapClose(enemies, EUnderTower);

            if (menu.GetComboBoxText("Hamle Modu: ") == "Fareye")
        ...
        public static void Flee()
        {
            ...
            if (menu.GetCheckboxValue("DuvardanGec") && Program.E.IsReady() && !YasuoCalcs.IsDashing())
            {
                //walldash
            ...
            if (menu.GetCheckboxValue("Kullan E") || activeDash != null)
            {
                if (activeDash == null)
                {
                    ...
                    EToMouse(menu.GetCheckboxValue("Kullan E Kule Alti"), menu.GetCheckboxValue("Yuk Kas Q"), false);
                }
                else
                {
                    ...
                    if (actualDashPosition.Distance(activeDash.endPosition) <= menu.GetSliderValue("DuvardanGecme Extra Uzaklik"))
        ...
        public static void CastItems(List<Obj_AI_Base> enemies, bool ks)
        {
            ...
            InventorySlot QSS = (MenuHandler.GetCheckboxValue(MenuHandler.Items, "Kullan Civali")) ? Yasuo.GetItem(ItemId.Quicksilver_Sash) : null,
                MercurialsScimitar = (MenuHandler.GetCheckboxValue(MenuHandler.Items, "Kullan Civa Yatagan")) ? Yasuo.GetItem(ItemId.Mercurial_Scimitar) : null,
                RavenousHydra = (MenuHandler.GetCheckboxValue(MenuHandler.Items, "Kullan Vahsi Hydra")) ? Yasuo.GetItem(ItemId.Ravenous_Hydra) : null,
                TitanicHydra = (MenuHandler.GetCheckboxValue(MenuHandler.Items, "Kullan Hasmetli Hydra")) ? Yasuo.GetItem(ItemId.Titanic_Hydra) : null,
                Tiamat = (MenuHandler.GetCheckboxValue(MenuHandler.Items, "Kullan Tiamat")) ? Yasuo.GetItem(ItemId.Tiamat) : null,
                Youmuus = (MenuHandler.GetCheckboxValue(MenuHandler.Items, "Kullan Youmuus")) ? Yasuo.GetItem(ItemId.Youmuus_Ghostblade) : null,
                BOTRK = (MenuHandler.GetCheckboxValue(MenuHandler.Items, "Kullan Mahvolmus")) ? Yasuo.GetItem(ItemId.Blade_of_the_Ruined_King) : null,
                BilgewaterCutlass = (MenuHandler.GetCheckboxValue(MenuHandler.Items, "Kullan Bilgewater Palasi")) ? Yasuo.GetItem(ItemId.Bilgewater_Cutlass) : null,
                HextechGunblade = (MenuHandler.GetCheckboxValue(MenuHandler.Items, "Kullan Hextech Silahkilic")) ? Yasuo.GetItem(ItemId.Hextech_Gunblade) : null;
            #endregion
        ...
        public static void UltHandler()
        {
            ...
            if (MenuHandler.Ult.GetCheckboxValue("Kullan R takip ederken") &&
                Yasuo.Mana != 100)
                CastR();

            if (MenuHandler.Ult.GetCheckboxValue("Kullan R butun dusmanlar menzildeyken") &&
                enemiesKnockedUp.Count() == Yasuo.CountEnemyHeroesInRangeWithPrediction((int)Program.R.Range, 0))
                CastR();

            if (MenuHandler.Ult.GetCheckboxValue("Kullan R at 10% HP") &&
                Yasuo.HealthPercent <= 0.10f &&
                Yasuo.Mana != 100)
                CastR(true);

            if (MenuHandler.Ult.GetCheckboxValue("Kullan R Secilen hedefe") &&
                enemiesKnockedUp.Any(a => a == TargetSelector.SelectedTarget))
                CastR(true);

            if (enemiesKnockedUp.Count() - 1 >= MenuHandler.Ult.GetSliderValue("Kullan R secilen hedef ve x daha fazla dusman varsa:") &&
                enemiesKnockedUp.Any(a=>a == TargetSelector.SelectedTarget))
                CastR(true);

            if (enemiesKnockedUp.Count() >= MenuHandler.Ult.GetSliderValue("Kullan R x dusman yada fazlasi icin:"))
                CastR(true);
        }
        ...
        public static void CastR(bool force = false)
        {
            ...
            if (!force && MenuHandler.GetCheckboxValue(MenuHandler.Ult, "Kullan R Son saniyede") &&
                timeTilLastEnemyIsPutDown <= 0.1f)
                    didActionThisTick = Program.R.Cast();
            else if (force || !MenuHandler.GetCheckboxValue(MenuHandler.Ult, "Kullan R Son saniyede"))
                    didActionThisTick = Program.R.Cast();;
        }
