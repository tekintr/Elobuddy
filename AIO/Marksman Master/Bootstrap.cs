    ...
    internal class Bootstrap
    {
            ...
            Core.DelayAction(
                () =>
                {
                    ...
                    Misc.PrintInfoMessage(
                        $"<b><font color=\"#5ED43D\">{Player.Instance.ChampionName}</font></b> yukleme basarili.Tekrar hosgeldin <b><font color=\"{(SandboxConfig.IsBuddy ? "#BF1B49" : "#1BBF91")}\">{(SandboxConfig.IsBuddy ? "[VIP] " + (SandboxConfig.Username == "intr" ? "intr you boosted animal from Latvia <3" : SandboxConfig.Username) : SandboxConfig.Username == "intr" ? "intr you boosted animal from Latvia <3" : SandboxConfig.Username)}</font></b> !");

                    Misc.PrintDebugMessage("Marksman AIO tamamen yuklendi");
                }, 250);
        }
        ...
        private static System.Version GetGithubVersion()
        {
            try
            {
                ...
                {
                    var downloadedData = webClient.DownloadString("https://raw.githubusercontent.com/tekintr/YeniTurkce/Marksman-AIO/master/Marksman%20Master/Marksman%20Master/Properties/AssemblyInfo.cs");
        ...
        private static void PrintVersionInfo()
        {
            try
            {
                ...
                if (version == 1)
                {
                    Misc.PrintInfoMessage("<i><red>Senin versiyonun eski. Yukleyiciden guncellemeyi deneyin.</red></i>");
                }
            }
            catch (Exception exception)
            {
