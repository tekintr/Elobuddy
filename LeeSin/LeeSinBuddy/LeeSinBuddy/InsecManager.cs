...
namespace LeeSinBuddy
{
    internal class InsecManager
    {
        ...
        {
            InsecMenu = Program.menu.AddSubMenu("Insec Ayarlari");
            InsecMenu.AddGroupLabel("Insec Ayarlari");
            InsecMenu.AddLabeledSlider("insecPositionMode", "Insec Pozisyon Ayari",
                new[] {"Takim konumuna", "Fare konumu", "Secilen birim"});
            InsecMenu.AddLabeledSlider("insecTargetMode", "Insec Hedefleme modu", new[] {"Tiklama ile", "Otomatik"});
            InsecMenu.Add("useFlash", new CheckBox("Sicra kullanarak insec yap?", false));
            InsecMenu.Add("checkAllUnits", new CheckBox("Insec icin diger birimleri kontrol et"));
            InsecMenu.Add("insecDistance", new Slider("Insec Mesafesi", 200, 100, 350));
            var insec = InsecMenu.Add("insecActive",
                new KeyBind("Insec Aktif Tusu", false, KeyBind.BindTypes.HoldActive, 'T'));
            var wtfsec = InsecMenu.Add("wtfsecActive",
                new KeyBind("WTFSec Aktif Tusu", false, KeyBind.BindTypes.HoldActive, 'Y'));
            ...
            Game.OnTick += delegate
            {
                ...
                {
                    if(Orbwalker.ActiveModesFlags.HasFlag(Orbwalker.ActiveModes.Flee)) Chat.Print("LeeSinBuddy - Kacis aktif! insec ile kacis tusu ani olamaz tuslari degistirin.", Color.BlueViolet);
                    InsecActive = true;
                    Insec();
                }
                else if (wtfsec.CurrentValue)
                {
                    if (Orbwalker.ActiveModesFlags.HasFlag(Orbwalker.ActiveModes.Flee)) Chat.Print("LeeSinBuddy - Kacis aktif! WTFSec ile kacis tusu ani olamaz tuslari degistirin.", Color.BlueViolet);
                    InsecActive = true;
                    WtfSecActive = true;
                    WtfSec();
                }
